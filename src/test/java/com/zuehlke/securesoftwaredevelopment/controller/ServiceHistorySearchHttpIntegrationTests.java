...
class ServiceHistorySearchHttpIntegrationTests {
    ...
    void normalHistorySearchReturnsOnlyAuthenticatedCustomersCompletedServices() throws Exception {
        mockMvc.perform(post("/api/my/service-history/search")
                ...
                .content("{\"showPerformedServices\":true,\"showUsedParts\":true}"))
    ...
    @Test
    void clientCannotReplaceOwnershipPredicateWithMongoOperator() throws Exception {
        mockMvc.perform(post("/api/my/service-history/search")
                .principal(bruce)
                .contentType(APPLICATION_JSON)
    ...
    @Test
    void clientCannotExpandProjectionWithArbitraryMongoFields() throws Exception {
        mockMvc.perform(post("/api/my/service-history/search")
                .principal(bruce)
                .contentType(APPLICATION_JSON)
                .content("{\"carModel\":\"Ford Focus\","
    ...
    @Test
    void typedDomainFiltersStillWork() throws Exception {
        mockMvc.perform(post("/api/my/service-history/search")
                .principal(bruce)
                .contentType(APPLICATION_JSON)
                .content("{\"carModel\":\"Ford Focus\",\"serviceName\":"
                        + "\"Engine oil and filter replacement\",\"showPerformedServices\":true}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1))
                .andExpect(jsonPath("$[0].serviceId").value(3))
                .andExpect(jsonPath("$[0].performedServices[0].name")
