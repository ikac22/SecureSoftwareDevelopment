...
RESULT_ROOT="$ROOT/attacks/cwe-78/evidence"
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/cwe78-experiment.XXXXXX")"
...
require_tag() {
  ...
  if ! git -C "$ROOT" rev-parse --verify --quiet "refs/tags/$tag^{commit}" >/dev/null; then
    ...
    echo "  bash attacks/cwe-78/tag-snapshots.sh --push" >&2
    exit 2
  fi
}
...
copy_evidence() {
  local worktree="$1"
  local phase="$2"
  local generated="$worktree/attacks/cwe-78/evidence/$phase"
  local destination="$RESULT_ROOT/$phase"

  mkdir -p "$destination"
  for file in request.txt response-headers.txt response.tar tar-contents.txt result.txt; do
...
run_phase() {
  ...
  echo "==> CWE-78 $phase: checking out tag $tag"
  ...
  (
    cd "$worktree"
    bash ./mvnw -B \
      -Dtest=com.zuehlke.securesoftwaredevelopment.controller.Cwe78ExperimentTests \
      -Dcwe78.phase="$phase" \
      test
  ...
  if [[ $status -ne 0 ]]; then
    echo "CWE-78 $phase experiment failed for tag $tag." >&2
    echo "See $evidence_dir/maven-output.txt" >&2
    exit $status
  fi

  echo "CWE-78 $phase experiment passed for tag $tag."
}
