...
RESULT_ROOT="$ROOT/attacks/cwe-94/evidence"
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/cwe94-experiment.XXXXXX")"
...
require_tag() {
  ...
  if ! git -C "$ROOT" rev-parse --verify --quiet "refs/tags/$tag^{commit}" >/dev/null; then
    ...
    echo "  bash attacks/cwe-94/tag-snapshots.sh --push" >&2
    exit 2
  fi
}
...
copy_evidence() {
  local worktree="$1"
  local phase="$2"
  local generated="$worktree/attacks/cwe-94/evidence/$phase"
  local destination="$RESULT_ROOT/$phase"

  mkdir -p "$destination"
  for file in baseline-request.txt baseline-response.json attack-request.txt attack-response.json result.txt; do
...
run_phase() {
  ...
  echo "==> CWE-94 $phase: checking out tag $tag"
  ...
  (
    cd "$worktree"
    bash ./mvnw -B \
      -Dtest=com.zuehlke.securesoftwaredevelopment.controller.Cwe94ExperimentTests \
      -Dcwe94.phase="$phase" \
      test
  ...
  if [[ $status -ne 0 ]]; then
    echo "CWE-94 $phase experiment failed for tag $tag." >&2
    echo "See $evidence_dir/maven-output.txt" >&2
    exit $status
  fi

  echo "CWE-94 $phase experiment passed for tag $tag."
}
...
if [[ "$MODE" == "all" ]]; then
  echo "CWE-94 comparison passed: the same test succeeds with vulnerable expectations on $VULNERABLE_TAG and mitigated expectations on $MITIGATED_TAG."
fi
