...
RESULT_ROOT="$ROOT/attacks/cwe-90/evidence"
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/cwe90-experiment.XXXXXX")"
...
require_tag() {
  ...
  if ! git -C "$ROOT" rev-parse --verify --quiet "refs/tags/$tag^{commit}" >/dev/null; then
    ...
    echo "  bash attacks/cwe-90/tag-snapshots.sh --push" >&2
    exit 2
  fi
}
...
copy_evidence() {
  local worktree="$1"
  local phase="$2"
  local generated="$worktree/attacks/cwe-90/evidence/$phase"
  local destination="$RESULT_ROOT/$phase"

  mkdir -p "$destination"
  for file in true-probe-request.txt true-probe-response.json false-probe-request.txt false-probe-response.json result.txt; do
...
run_phase() {
  ...
  echo "==> CWE-90 $phase: checking out tag $tag"
  ...
  if [[ $status -ne 0 ]]; then
    echo "CWE-90 $phase experiment failed for tag $tag." >&2
    echo "See $evidence_dir/maven-output.txt" >&2
    exit $status
  fi

  echo "CWE-90 $phase experiment passed for tag $tag."
}
